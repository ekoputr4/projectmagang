...
class GroupKerja
{
	...
	function cekVariabel($db)
	{
	   ...
	   $example->add_text_field("Nama", $this->group_name, "text", "y", 30, 2);
	...
	function existGroupKerja($txt_id,$db) 
	{
    ...
    function getGroupKerja($txt_id,$db) 
    {
       if (! $this->existGroupKerja($txt_id,$db)) {
           return  False;
       } else {
	   	   $this->getData();
	       return True;
	...
	function delGroupKerja($txt_id,$db) 
	{
       if ($this->existGroupKerja($txt_id,$db)) {
	       $sqlstr = "select group_id from subgroupkerja where group_id='".safeString($txt_id)."'";
		   $result = mysql_query($sqlstr,$db);
		   if (mysql_num_rows($result)==0) {
		      ...
		      $this->error = "GroupKerja masih digunakan pada SubGroup";
			  return False;
		   }						
	   } else {
	...
	function saveGroupKerja($db) 
	{   
	  $this->filterGroupKerja();   
	  if ($this->cekVariabel($db))  {
		  if ($this->existGroupKerja($this->group_id,$db)) {
